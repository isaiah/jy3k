...
public class PyException extends RuntimeException implements Traverseproc
{
    ...
    public boolean match(PyObject exc) {
        ...
        } else if (exc != null && !isExceptionClass(exc)) {
        ...
        // it right in 1.1
        if (exc == Py.IOError) {
            if (Py.isInstance(value, PyType.fromClass(IOException.class))) {
                return true;
            }
        }
        // FIXME too, same approach for OutOfMemoryError
        if (exc == Py.MemoryError) {
            if (Py.isInstance(value, PyType.fromClass(OutOfMemoryError.class))) {
                return true;
            }
        }
