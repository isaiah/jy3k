...
public class PyScanner extends PyObject implements Traverseproc {
    public static final PyType TYPE = PyType.fromClass(PyScanner.class);
    ...
    static final PyObject Scanner___new__(PyNewWrapper new_, boolean init, PyType subtype,
        ...
        return new PyScanner(args[0]);
    }

    @ExposedMethod
    public PyObject __call__(PyObject string, PyObject idx) {
        return _scan_once((PyUnicode) string, idx.asInt());
    }
    ...
    public PyTuple _parse_object(PyUnicode pystr, int idx) { // }, Py_ssize_t *next_idx_ptr) {
    ...
    public PyTuple _parse_array(PyUnicode pystr, int idx) {
    ...
    public PyTuple _scan_once(PyUnicode pystr, int idx) {
    ...
    public PyTuple _match_number(PyUnicode pystr, int start) {
        ...
        */
        ...
        PyUnicode numstr = (PyUnicode) str.getslice(start, idx);
        if (is_float) {
