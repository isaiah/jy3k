...
public class PyEncoder extends PyObject implements Traverseproc {
    public static final PyType TYPE = PyType.fromClass(PyEncoder.class);
    final PyObject markers;
    ...
    public PyEncoder(PyObject[] args, String[] kwds) {
        super(TYPE);
    ...
    static final PyObject Encoder___new__(PyNewWrapper new_, boolean init, PyType subtype,
        ...
        return new PyEncoder(args, keywords);
    }
    ...
    private void encode_obj(PyList rval, PyObject obj, int indent_level) {
        ...
        if (obj == Py.None) {
            rval.append(new PyUnicode("null"));
        } else if (obj == Py.True) {
            rval.append(new PyUnicode("true"));
        } else if (obj == Py.False) {
            rval.append(new PyUnicode("false"));
